...
- Vertex input binding descriptions (*i.e.* vertex buffer binding point,
...
- Pipeline layout, including descriptor layouts and push constant ranges used
in this pipeline. For this simple scene, we don't use descriptors. We only need
to pass the `VkPushConstantRange`.
- Viewport transform. Since this may change when the window is resized, we need
to set it every time when we build/rebuild the pipeline.
- Color blending for each color attachment.
