...
#include "jessie_engine/common/camera.h"
...
void NanosuitApp::Init() {
  if (is_first_time) {
    ...
    cube_model_.Init(context_->ptr(),
                     "jessie_engine/resource/model/cube.obj", 1);
    skybox_model_.Init(context_->ptr(),
                       "jessie_engine/resource/model/skybox.obj", 1);
    ...
    // texture
    cube_tex_.Init(context_, {"jessie_engine/resource/texture/statue.jpg"});
    const std::string skybox_dir{"jessie_engine/resource/texture/tidepool/"};
  ...
  cube_pipeline_.Init(
      context_->ptr(),
      {{"jessie_engine/shader/compiled/simple.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT},
       {"jessie_engine/shader/compiled/simple.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT}},
  ...
  skybox_pipeline_.Init(
      context_->ptr(),
      {{"jessie_engine/shader/compiled/skybox.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT},
       {"jessie_engine/shader/compiled/skybox.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT}},
